...
import { Container, Navbar, Nav, Button, Form, Card, Row, Col, Badge, ListGroup, Alert } from 'react-bootstrap';
...
export default function App() {
  ...
  const [view, setView] = useState<'landing' | 'login' | 'register' | 'dashboard' | 'course-detail'>('landing');
                          ...
                          <Card className="h-100 border-0 shadow-sm">
                            <Card.Body className="d-flex flex-column">
                              ...
                              <div className="mt-auto">
                                <Button variant="primary" className="w-100" onClick={() => setView('register')}>Saber más</Button>
                              </div>
                            </Card.Body>
                          </Card>
                        </Col>
                ...
                <Row className="g-4">
                  <Col lg={8}>
                    <section className="mb-5">
                      <h3 className="fw-bold h5 mb-4">Mis Cursos Matriculados</h3>
                      <Row className="g-3">
                        {myCourses.length > 0 ? myCourses.map(course => (
                          <Col sm={6} key={course.id}>
                            <Card className="h-100 border-0 shadow-sm cursor-pointer hover-card" onClick={() => viewCourseDetail(course.id)}>
                          ...
                          <Col xs={12}>
                            <Alert variant="light" className="text-center py-5 border-2 border-dashed">
                              <p className="text-muted mb-0">Aún no estás matriculado en ningún curso.</p>
                            </Alert>
                          </Col>
                        )}
                      </Row>
